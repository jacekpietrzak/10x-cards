...
import { DeleteFlashcardResponseDto } from "@/lib/types";
...
export async function GET(
  ...
  try {
    ...
    const flashcard = await getFlashcardById(id, authResult.userId, supabase);
...
export async function PUT(
  ...
  try {
    ...
    if (err instanceof Error && err.message.includes("Invalid generation_id")) {
 ...
 * Deletes a single flashcard belonging to the authenticated user
 */
export async function DELETE(
  _request: NextRequest,
  context: { params: Promise<{ id: string }> },
): Promise<NextResponse<DeleteFlashcardResponseDto | { error: string }>> {
  if (!isFeatureEnabled("flashcards")) {
    return NextResponse.json(
      { error: "Flashcards feature is currently disabled" },
      { status: 503 },
  ...
  try {
    ...
    // 3. Delete the flashcard using service layer
    const deleted = await deleteFlashcard(id, authResult.userId, supabase);
