...
export function useReviewSession() {
  ...
  const convertToFSRSCard = useCallback((flashcard: FlashcardDto) => {
    ...
    if (isMissingCriticalParams) {
      ...
      const safeDueDate =
        flashcard.due && flashcard.due !== null
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      const { data: response } = await request(`/api/flashcards?${params}`, {
        method: "GET",
      });
  ...
  const rateCard = useCallback(
    async (rating: FSRSGrade) => {
      ...
      try {
        ...
        if (rating === 1 && (fsrsCard.state === 2 || fsrsCard.state === 3)) {
          finalLapses = Math.max(newCard.lapses, (currentCard.lapses || 0) + 1);
        }
