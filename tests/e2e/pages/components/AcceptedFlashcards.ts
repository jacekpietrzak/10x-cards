...
export class AcceptedFlashcards {
  ...
  constructor(page: Page) {
    this.page = page;
    this.container = page.locator('[data-test-id="accepted-flashcards-container"]');
    this.saveButton = page.locator('[data-test-id="bulk-save-flashcards-button"]');
    this.acceptedItems = this.container.locator('[data-test-id="flashcard-proposal-item"]');
  ...
  async waitForSaveComplete() {
    ...
    await this.page.waitForResponse(
      response => response.url().includes('/api/flashcards') && 
  ...
  async moveBackToProposals(index: number) {
    ...
    if (flashcards[index]) {
      const acceptButton = flashcards[index].locator('[data-test-id="flashcard-accept-button"]');
  ...
  async removeFromAccepted(index: number) {
    const flashcards = await this.getAllAcceptedFlashcards();
    if (flashcards[index]) {
      const rejectButton = flashcards[index].locator('[data-test-id="flashcard-reject-button"]');
  ...
  async editAcceptedFlashcard(index: number) {
    const flashcards = await this.getAllAcceptedFlashcards();
    if (flashcards[index]) {
      const editButton = flashcards[index].locator('[data-test-id="flashcard-edit-button"]');
  ...
  async getAcceptedFlashcardContent(index: number) {
    const flashcards = await this.getAllAcceptedFlashcards();
    if (flashcards[index]) {
      const front = await flashcards[index].locator('.bg-muted').nth(0).textContent();
      const back = await flashcards[index].locator('.bg-muted').nth(1).textContent();
      return { front, back };
    }
    return null;
  }
