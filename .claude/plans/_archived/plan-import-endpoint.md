...
**2026-05-28 — Production deployed + verified end-to-end. Plan COMPLETE.** Pushed all 11 commits to `main`; GH Actions "Production Deployment" workflow shipped the import endpoint to https://10x-cards.jackpietrzak.com. Smoke test (manual `curl`): new front → `{"inserted":1,"updated":0}` HTTP 200; re-import same front, new back → `{"inserted":0,"updated":1}` HTTP 200; the upserted row appeared in My Flashcards for `jacekpietrzak@me.com` with `Source: Manual` and correct timestamps — confirming bearer auth, middleware allowlist, service-role admin client, FK to `auth.users`, FSRS defaults, and upsert dedup all work in production exactly as designed. The "Not yet production-verified" caveat from Step 6 is now satisfied.

**Two deploy-time discoveries (folded as docs in repo CLAUDE.md files, not the plan):**
- **Vercel's "Redeploy" button does not work for this project.** Vercel actively blocks it because deploys use `vercel deploy --prebuilt --prod` ([master.yml:111](../../../.github/workflows/master.yml#L111)) — prebuilt artifacts have env vars baked in at build time. Env-var refreshes require `git commit --allow-empty && git push` to re-trigger the GH Actions workflow (which re-runs `vercel pull` with the latest values). Now documented in [10x-cards CLAUDE.md `## Deployment`](../../../CLAUDE.md). The plan's redeploy-step-4 correction (folded post-Step-6 review) was correct in intent and turned out to be more load-bearing in practice than the review estimated — without it, the deploy would have stalled at the Vercel UI.
- **`SUPABASE_SERVICE_ROLE_KEY` value in Vercel was stale, not missing.** First production attempt returned 500 with `Invalid API key` from Supabase. Root cause: Supabase rolled out new opaque API keys (`sb_publishable_*` / `sb_secret_*`) replacing the JWT `anon` / `service_role` keys, and the old service_role JWT had been invalidated server-side. Fix: created a new Supabase Secret API key (`sb_secret_*`) and updated the existing `SUPABASE_SERVICE_ROLE_KEY` env-var **value** in Vercel — env-var **name** unchanged, no code change required, since supabase-js (≥ 2.45; project uses 2.49.4) accepts the new format as a drop-in. This vindicates the Step 6 NOTE flagging `SUPABASE_SERVICE_ROLE_KEY` as risky-during-deploy: it WAS the issue, just with a different root cause than predicted (stale value, not missing entirely).

**Knowledge migration (post-deploy hygiene).** The three machine-local memory files written during this plan (`prebuilt-deploy-quirk`, `user_email_distinction`, `verify-now-over-deferring`) were migrated out of `/root/.claude/projects/-home-projects-10x-cards/memory/` (which is machine-local and unversioned — knowledge there evaporates when the user switches machines) into versioned repo `CLAUDE.md` files: the deploy-quirk to [10x-cards CLAUDE.md](../../../CLAUDE.md) `## Deployment` section, the other two to `life-os/CLAUDE.md §VIII (For AI Assistants)`. A new `### Memory & Persistence Defaults` subsection in life-os/CLAUDE.md captures the *generative* rule (default durable knowledge to repo CLAUDE.md files, not machine-local memory) so this isn't a repeat exercise — future memory-temptations get routed correctly from the start.

**Plan COMPLETE.** Moving the plan file to `.claude/plans/_archived/plan-import-endpoint.md`. Only user-side wrap-ups remain — all operational, none in plan scope: delete the `prod-smoke-test` row from My Flashcards UI, save `IMPORT_API_KEY` in a password manager (Vercel masks it after save, no recovery), and wire the PKS tool against `POST /api/import` with the documented contract.
