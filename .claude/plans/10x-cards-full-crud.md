...
| 3 | Route handler wiring + maxDuration | API | 1 h | No | 2 | Shipped |
| 4 | Unit tests (route + service) | tests | 2 h | No | 3 | Shipped |
| 5 | Smoke test extension + docs | scripts/docs | 1 h | No | 3 | Shipped |
...
**2026-07-19 — PostgREST query semantics VERIFIED (the plan's last untested library assumption; closed during the Step 3 review).** Both query shapes the Step 2 service depends on are confirmed against the *installed* `postgrest-js@1.19.4` typings, not from memory: (1) `.delete().eq(…).select("id")` returns the deleted rows — `delete()` types `Result` as `null` with the doc comment "By default, deleted rows are not returned. To return it, chain the call with `.select()` after filters", and the chained `.select()` flips `Result` to `NewResultOne[]` while appending `return=representation` to the Prefer header at runtime, so `data.length` is an exact deleted-row count; (2) `.neq()` is on `PostgrestFilterBuilder` and returns `this`, chaining freely before `.limit(1).maybeSingle()`. Decisive corroboration: the identical `.delete().eq(…).select("id")` + `data?.length` pattern already ships in three sibling call sites at [api/test/cleanup/route.ts:59](src/app/api/test/cleanup/route.ts#L59), [:72](src/app/api/test/cleanup/route.ts#L72), [:85](src/app/api/test/cleanup/route.ts#L85) — pre-existing repo precedent, not a novel pattern. Treat as closed: no live-DB run is needed to de-risk it, and Step 5's smoke test is end-to-end confirmation rather than first proof.

**2026-07-19 — Step 3 shipped.** Rewired [src/app/api/import/route.ts](src/app/api/import/route.ts) to call `processImportRequest(result.data, …)`, removing Step 1's transitional `cards ?? []` shim — delete/patch-only requests are no longer 200 no-ops. Added `export const maxDuration = 60;` and rewrote the route doc comment (three request arrays, five-field response, delete → patch → upsert order, `source='manual'` invariant, full error-code table). Auth/config/JSON-parse/Zod blocks verified **byte-identical** via a targeted diff filter, honoring the `timingSafeEqual` pitfall. Substep 3.1 done: [route.test.ts](src/app/api/import/__tests__/route.test.ts) now mocks `processImportRequest`, with an `EMPTY_RESULT` five-field helper and the two arg-shape assertions retargeted from `cardsArg` to `payloadArg`. Verified: 72/72 tests, lint clean, prod build passes. Additionally ran a throwaway integration test (real route + real orchestrator, only the DB client stubbed against an in-memory table; 9 scenarios — five-field shape, delete/patch-only now functional, `ai-full` row survives `delete_fronts`, both skip reasons, phase order, all-empty 400, smoke-script key order — deleted before commit). Also **verified rather than assumed** that the legacy smoke script stays green: its `expect()` uses `grep -qF` substring matching, so `"inserted":1,"updated":0` still matches the five-field body. /plan-review delta pass: GO — one WARNING (below), the PostgREST assumption closed as verified (see the entry above), and two Step 4/5 notes folded inline. **Up next:** step 4.

**Deploy-risk note for the Step 3 commit:** this is the first commit in the plan that can go red *after* lint and tests pass. `maxDuration = 60` is the repo's first such export (no `vercel.json`; even `/api/generations` relies on defaults), and `npm run build` does **not** validate it — only `vercel build --prod` / deploy does, at stage 3–4 of `.github/workflows/master.yml`. Watch the GitHub Actions run after pushing; if the tier rejects it, drop to the tier maximum rather than removing the export.

**2026-07-19 — Step 4 shipped.** All 8 matrix items covered, split exactly per the Step-3 note: [route.test.ts](src/app/api/import/__tests__/route.test.ts) (+176 lines — backward compat, all validation 400s incl. post-trim overlap and both >100 caps, delete/patch-only pass-through, combined pass-through), [flashcards.service.test.ts](src/lib/services/__tests__/flashcards.service.test.ts) (+492/−16 — mock extended per the Pitfalls spec with `deletes` capture, `.neq()`, delete-op thenable returning `{data,error}`, `select()` guarded after `delete()`; lookups capture reshaped to `{eqCalls,neqCalls}`; new describes for delete/patch/orchestrator incl. both skip reasons, back-only patch, source-scoping assertions, dedup warns, error rethrows, exact five-field key order, `opOrder` phase sequence), plus the recommended third file [route.integration.test.ts](src/app/api/import/__tests__/route.integration.test.ts) (real route + real orchestrator over an in-memory table via `vi.hoisted` — smoke-grep substring pinned on raw text, `ai-full` rows survive delete/patch, other-user rows untouched, id preserved on rename, patch-frees-front-then-upsert phase-order proof, combined compose). Verified: 107/107 tests (was 72), lint clean, prod build passes. Zero production code changed — commit is deploy-safe. /plan-review delta pass: GO, two doc-grade NOTEs — mock "filters recorded, not applied" caveat folded as a test-file doc comment; exact `opOrder` coupling to upsert internals accepted (integration file proves phase order behaviorally). Real-DB closure (updated_at trigger, end-to-end) remains Step 5's smoke test. **Up next:** step 5.

**2026-07-19 — Step 5 shipped; plan complete.** Extended [scripts/import-smoke-test.sh](scripts/import-smoke-test.sh) from 6 to 10 checks (patch rename → `patched:1`, conflicting rename → `new_front_conflict` in `skipped_patches`, delete both smoke rows → `deleted:2`, idempotent re-delete → `deleted:0` proving zero residue) — the script now cleans up after itself; header rewritten accordingly (early exit 1 mid-run can still leave ≤2 rows). Updated [DEPLOYMENT.md](DEPLOYMENT.md): new "Kontrakt endpointu" section (three arrays, five-field response, skip reasons, rename-chain tail-first guidance, idempotent-retry recovery, disjointness-400 folded from the closing review), smoke-test section now leads with the self-cleaning script. **Verified beyond plan acceptance:** Docker + local Supabase brought up this session — all 10 checks green against the real route + real Postgres (not just the contract stub), `updated_at` trigger confirmed firing on patch, and an `ai-full` row with its front in `delete_fronts` confirmed surviving (the `source='manual'` invariant against real data). 107/107 tests, lint clean. /plan-review closing delta: GO — one NOTE folded (disjointness line in DEPLOYMENT.md), one transitional NOTE (running the new script against prod **before** these 10 commits deploy fails at check 7 and leaves 2 rows — deploy first, then smoke-test). Remaining prod-only unknown: `maxDuration = 60` at deploy time (see the Step-3 deploy-risk note — watch the GitHub Actions run). **Up next:** none — all 5 steps shipped; plan complete.
...
**Note (added 2026-07-19 Step-3 review): the matrix spans two files, and the split is not optional.** Because the route now mocks the orchestrator wholesale, items **2, 3, 4, 5, 8 are unreachable from `route.test.ts`** — they can only be asserted in `flashcards.service.test.ts` against the chainable client mock. Items **6 and 7 are route-level** (backward compat, validation 400s, since Zod runs before the orchestrator is called). Item 1 straddles both: assert counts pass-through at the route, per-phase behavior at the service. Don't try to assert `.eq("source","manual")` through a mocked orchestrator.

**Note (added 2026-07-19 Step-3 review): consider a third, integration-shaped test file.** Step 3's throwaway proved its worth — real route + real `processImportRequest`, with only `createAdminClient` mocked to return a builder over an in-memory rows array. It covers the seam neither planned file reaches: mocked-route tests assert *that* the orchestrator is called, service tests assert the phases, but neither asserts that the route hands `result.data` through and the phases compose into the five-field body. It also pins the smoke-script key order end to end. ~40 lines; the builder stub needs `select/insert/update/delete/eq/neq/limit/maybeSingle` plus a thenable, and `select()` must not overwrite `op` when it follows `delete()`.
...
- Docs: update `DEPLOYMENT.md`'s `/api/import` section and the endpoint contract wherever it's documented; no CLAUDE.md env changes (no new env vars). **Note (added 2026-07-19 Step-3 review):** the concrete drift is [DEPLOYMENT.md:63-64](DEPLOYMENT.md#L63-L64), which documents `{"inserted":1,"updated":0}` as the expected body — now a strict subset of the real five-field response. Also fold in the rename-chain ordering guidance from Step 2 (order chains tail-first, or expect convergence on the second push).
- **Note (added 2026-07-19 Step-3 review):** existing checks 3/4/5a/5b need **no change** — `expect()` matches with `grep -qF`, so `"inserted":1,"updated":0"` still hits inside the five-field body (verified against the script, not assumed). Extend the script; don't rewrite it.
