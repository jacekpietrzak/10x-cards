...
describe("GenerationService", () => {
  ...
  describe("validateFlashcards", () => {
    ...
    it("should throw error for missing front field", () => {
      const invalidCards = [{ back: "Answer only" }];

      expect(() => service.validateFlashcards(invalidCards)).toThrow(
        "Invalid flashcard: missing front text",
      );
    });
  });
});
...
_Ostatnia aktualizacja: Grudzień 2024 - po implementacji 57 testów jednostkowych_
